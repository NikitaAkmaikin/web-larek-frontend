...
events.on ('order:changed', () => {
  order.valid = order.getAddress() && order.isSelected();
})

// ИЗМЕНЕНИЕ СОСТОЯНИЯ ФОРМЫ С КОНТАКТНЫМИ ДАННЫМИ
events.on ('contacts:changed', () => {
  contact.valid = contact.getEmail() && contact.getPhone();
});

// Изменение состояния валидации форм
events.on('formErrors:change', (errors: Partial<IOrderForm>) => {
  const { email, phone, address } = errors;
  order.errors = contact.errors = Object.values({email, phone, address}).filter(i => !!i).join('; ');
});
